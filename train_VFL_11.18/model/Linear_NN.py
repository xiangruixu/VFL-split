...
class Client_LinearNet_1_em_bm(nn.Module):
    def __init__(self, in_dim=7, n_hidden_1=1024, n_hidden_2=64, client=1):
        super(Client_LinearNet_1_em_bm, self).__init__()


        # self.embedding_layer = nn.Embedding.from_pretrained(a)
        if client ==1:
            self.layer1 = nn.Sequential(nn.Linear(10, n_hidden_2),
                                    )

        if client ==2:
            self.layer1 = nn.Sequential(nn.Linear(16, n_hidden_2),
                                    )

        if client ==3:

            self.layer1 = nn.Sequential(nn.Linear(31, n_hidden_2),
                                   )
