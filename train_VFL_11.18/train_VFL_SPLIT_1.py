...
def train_server(client1_fx, client2_fx, client3_fx, y, t, i, batch,correct):
    server_model.train()
    correct = correct
    size = size_train
    ...
    #   print('server_model.p', param)
    print('train--fx_server', fx_server)
    print('train--fx_server.norm()', fx_server.norm())
    ...
    return client1_fx.grad, client2_fx.grad, client3_fx.grad, correct
...
def train_client(dataloader, client_model_1, client_model_2, client_model_3,  t):
    ...
    for batch, (X, y) in enumerate(dataloader):
      ...
      for i in range(1):
        ...
        print('before--fx1', fx1)
        print('before--norm(fx1)', fx1.norm())
        ...
        print('after--fx1', client1_fx)
        print('after--norm(fx1)', client1_fx.norm())
        ...
        print('before--fx2', fx2)
        print('before--norm(fx2)', fx2.norm())
        ...
        print('after--fx2', client2_fx)
        print('after--norm(fx2)', client2_fx.norm())
        ...
        client3_fx = (fx3 + noise3).clone().detach().requires_grad_(True)


        # Sending activations to server and receiving gradients from server
        g_fx1, g_fx2, g_fx3, correct = train_server(client1_fx, client2_fx, client3_fx, y, t, i, batch, correct)   
        print('train--g_fx1', g_fx1)
        print('train--g_fx2', g_fx2)

  
        # backward prop
        (client1_fx).backward(g_fx1) # 不允许张量对张量求导，只允许标量对张量求导：将所有张量元素加权求和转换为标量
        (client2_fx).backward(g_fx2) # 因此分成两步：l = torch.sum(fx1*g_fx1) , 然后l 对 w求导；
        (client3_fx).backward(g_fx3) 
...
def test_server(client1_fx, client2_fx, client3_fx, y, batch, correct):
    ...
    # eval
    fx_server = server_model(client1_fx, client2_fx, client3_fx)
    print('test--fx_server', fx_server)
    print('test--fx_server.norm()', fx_server.norm())
    loss = criterion(fx_server, y)
    correct += (fx_server.argmax(1) == y).type(torch.float).sum().item()
